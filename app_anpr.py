...
class ClientApp:
    def __init__(self):
        self.img_path = "static/predictions/prediction.jpg"
    ...
    if request.method == 'POST' and 'imag' in request.files:
        ...
        if img.filename=='':
            ...
            if img and img.filename.rsplit('.',1)[1].lower() in ['jpg','jpeg','png']:
                ...
                img_vec = cv2.imdecode(npimg,  cv2.IMREAD_COLOR)
                # prediction
                pred_img,txt,scr = show_inference(capp.model,img_vec)
                # saving the prediction
                cv2.imwrite(capp.img_path,pred_img)

                flash('Succesfully made the predictions')
                return render_template('home.html',title="Vehicle Object Detection",filename="predictions/prediction.jpg",results = {'text':txt,'score':scr},lenn = len(txt))
